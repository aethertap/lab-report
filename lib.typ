...
#let report(
  ...
  // DOCUMENT METADATA
  set document(title: [Biology Lab, 2025], 
  ...
  v(2mm)
  h(5mm) 
  text(size: 48pt, weight: "bold", "Biology Lab") 
  h(45mm) 
  box(image("media/ginosterous.png", width: 20%, fit: "contain"), baseline: 18mm)
  v(-12mm)
  line(length: 11cm, stroke: 0.1cm)
  v(12mm)
  h(5mm) 
  box(text(size: 36pt, [*Ginosterous Academy*]))
  v(12mm)
  align(center, text(size: 20pt, [*Biology Lab Report*]))
  ...
  table(
    ...
    align: left,
    row-gutter: 4mm,
    [*Lab ID:*],                [#task-no: #task-name],
    [*Authors:*],  [#authors.join(", ")],
    [*Emails:*], [#mail-links.join(" ")],
    [*Date:*],                [#date]
  )
  ...
  v(4mm)
  h(5mm)
  text(size: 18pt, [*Ginosterous Academy*])
  linebreak()
  h(5mm)
  text(size: 18pt, [*science - math - engineering - art*])
  pagebreak()
    ...
    if heading.level == 1 {    
      ...
      if heading.supplement == [Appendiks] {
        link(heading.location(), text(fill: rgb("#000"), 
            [*Appendix #it.body #box(width: 1fr,  it.fill) #it.page*]))
      } else {
        link(heading.location(), text(fill: rgb("#000"),
...
#let appendices(body) = {
  ...
  show heading: it => {
    v(19pt)
    if it.level == 1 {
      text(weight: "semibold")[Appendix #counter(heading).display() #it.body]
    }
    else {
